  ...
  return (
    <div className="bg-linear-to-b from-white from-50% to-gray-100 pt-32 pb-32">
      <Container>
        ...
        <Lead className="mt-6 max-w-3xl">{lead}</Lead>
            ...
            {bullets.length > 0 && (
              <ul className={`space-y-3 ${paragraphs.length > 0 ? 'mt-8' : ''}`}>
                {bullets.map((bullet, index) => (
                  <li key={index} className="flex items-baseline gap-3 text-sm/6 text-gray-600">
                    <span className="size-1.5 shrink-0 rounded-full bg-blue-600 translate-y-[-2px]" />
                    {bullet}
                  </li>
                ))}
              </ul>
