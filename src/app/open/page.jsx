  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (data.user && !data.session) {
        ...
        // The user can complete onboarding, but may need to confirm email later
      }

      // Create lead with email after successful signup
      try {
        await createLead(email)
      } catch (leadError) {
        // Don't block the user flow if lead creation fails
        console.error('Lead creation error (non-blocking):', leadError)
      }

      // Trigger signup webhook with email (fire and forget)
      try {
      ...
      <div className="isolate flex min-h-dvh items-center justify-center p-6 lg:p-8">
        <div className="w-full max-w-md rounded-xl bg-white shadow-md ring-1 ring-black/5">
          <form onSubmit={handleSubmit} className="p-7 sm:p-11">
            ...
            <h1 className="mt-8 text-base/6 font-medium">Open your account</h1>
            <p className="mt-1 text-sm/5 text-gray-600">
              Create your Request Finance account to get started.
            </p>
            {error && (
